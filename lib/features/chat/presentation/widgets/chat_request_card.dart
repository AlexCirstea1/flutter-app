  ...
  Future<void> _decryptMessage() async {
    try {
      if (widget.dto.ciphertext.isNotEmpty) {
        // Get private key from storage service
        final storageService = widget.avatarService.storageService;
        final privateKey = await storageService.getPrivateKey();

        if (privateKey != null) {
          // Use MessageCryptoService to decrypt
          final cryptoService = MessageCryptoService();
          final decryptedText = cryptoService.decryptMessage(
            ciphertext: widget.dto.ciphertext,
            iv: widget.dto.iv,
            encryptedKey: widget.dto.encryptedKeyForRecipient,
            privateKey: privateKey,
          );

          if (mounted) {
            setState(() {
              _message = decryptedText ?? "[Failed to decrypt message]";
            });
          }
        } else {
          setState(() {
            _message = "[No private key available]";
          });
        }
      }
    } catch (e) {
      ...
      if (mounted) {
        setState(() {
          _message = "[Error decrypting message]";
        });
      }
    }
  }
              ...
              children: [
                Container(
                  ...
                  child: FutureBuilder<Uint8List?>(
                    future:
                        widget.avatarService.getAvatar(widget.dto.requester),
