...
import 'package:vaultx_app/core/data/services/websocket_service.dart';
...
class _ChatRequestsPageState extends State<ChatRequestsPage> {
  ...
  void _decide(ChatRequestDTO req, bool accept) async {
    ...
    if (accept) {
      await _chatService.acceptChatRequest(req.id);
    } else {
      await _chatService.rejectChatRequest(req.id);
    }
    if (mounted) {
